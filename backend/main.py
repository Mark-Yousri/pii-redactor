...
async def debug_ocr(file: UploadFile = File(...)):
    ...
    from backend.detect.text_extractor import extract_tokens_ocr, _find_card_bbox
    ...
    bbox = _find_card_bbox(img_arr)
    tokens = extract_tokens_ocr(img_arr, page_num=0, dpi=150)

    return {
        "original_size": [w, h],
        "card_detected": bbox is not None,
        "card_bbox_xywh": list(bbox) if bbox else None,
        "total_tokens": len(tokens),
        "tokens": [{"text": t.text, "conf_source": t.source, "bbox": [round(v,1) for v in t.bbox]} for t in tokens],
    }
